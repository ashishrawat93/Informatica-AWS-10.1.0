...
function createPCServices() {
    ...
    ($out = C:\Informatica\9.6.1\isp\bin\infacmd createRepositoryService -dn $env:DOMAIN_NAME  -nn $env:NODE_NAME -sn $env:REPOSITORY_SERVICE_NAME -so DBUser=$env:REPO_USER DatabaseType=$env:DB_TYPE DBPassword=$repopass ConnectString=$env:DB_SERVICENAME CodePage="UTF-8 encoding of Unicode" OperatingMode=NORMAL -un $env:DOMAIN_USER -pd $domainpass -sd ) | Out-Null
    $code=$LASTEXITCODE
    ac C:\InfaServiceLog.log $out 
    if ($env:SINGLE_NODE -eq 1 ) {
        ($out = C:\Informatica\9.6.1\isp\bin\infacmd createintegrationservice -dn $env:DOMAIN_NAME -nn $env:NODE_NAME -un $env:DOMAIN_USER -pd $domainpass -sn $env:INTEGRATION_SERVICE_NAME -rs  $env:REPOSITORY_SERVICE_NAME -ru $env:DOMAIN_USER -rp $domainpass  -po codepage_id=106 -sd -ev INFA_CODEPAGENAME=UTF-8) | Out-Null
        $code=$code -bor $LASTEXITCODE
        ac C:\InfaServiceLog.log $out 
    } else {
       ($out = C:\Informatica\9.6.1\isp\bin\infacmd creategrid -dn $env:DOMAIN_NAME -un $env:DOMAIN_USER -pd $domainpass -gn $env:GRID_NAME -nl $env:NODE_NAME) | Out-Null        

       $code = $LASTEXITCODE

        ac C:\InfaServiceLog.log $out 

       ($out = C:\Informatica\9.6.1\isp\bin\infacmd createintegrationservice -dn $env:DOMAIN_NAME -gn $env:GRID_NAME -un $env:DOMAIN_USER -pd $domainpass -sn $env:INTEGRATION_SERVICE_NAME -rs  $env:REPOSITORY_SERVICE_NAME -ru $env:DOMAIN_USER -rp $domainpass  -po codepage_id=106 -sd -ev INFA_CODEPAGENAME=UTF-8) | Out-Null

        $code = $code -bor $LASTEXITCODE

        ac C:\InfaServiceLog.log $out 

        ($out = C:\Informatica\9.6.1\isp\bin\infacmd updateServiceProcess -dn $env:DOMAIN_NAME -un $env:DOMAIN_USER -pd $domainpass -sn $env:INTEGRATION_SERVICE_NAME -nn $env:NODE_NAME -po CodePage_Id=106) | Out-Null
...
if ($env:JOIN_DOMAIN -eq 0 ) {
     ...
     ($out = C:\Informatica\9.6.1\isp\bin\infacmd updategrid -dn $env:DOMAIN_NAME -un $env:DOMAIN_USER -pd $domainpass -gn $env:GRID_NAME -nl  $env:JOIN_NODE_NAME -ul) |Out-Null
     $code = $LASTEXITCODE
     ac C:\InfaServiceLog.log $out
     ($out = C:\Informatica\9.6.1\isp\bin\infacmd updateServiceProcess -dn $env:DOMAIN_NAME -un $env:DOMAIN_USER -pd $domainpass -sn $env:INTEGRATION_SERVICE_NAME -nn $env:JOIN_NODE_NAME -po CodePage_Id=106 -ev INFA_CODEPAGENAME=UTF-8) | Out-Null
     $code = $code -bor $LASTEXITCODE
     ac C:\InfaServiceLog.log $out
     exit $code
}

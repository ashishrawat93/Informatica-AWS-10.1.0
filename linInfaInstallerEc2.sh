...
USER_INSTALL_DIR="\/home\/ec2-user\/Informatica\/9.6.1"
KEY_DEST_LOCATION="\/home\/ec2-user\/Informatica\/9.6.1\/isp\/config\/keys"

function setENV {
    echo "Setting env"
    ORACLE_HOME=/home/oracle/product/11.2.0
    LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/home/oracle/product/11.2.0/lib
...
function createPCServices {
	...
	then
	  ...
	  sh /home/ec2-user/Informatica/9.6.1/isp/bin/infacmd.sh  creategrid -dn $DOMAIN_NAME -un $DOMAIN_USER -pd $domainpass -gn $GRID_NAME -nl $NODE_NAME &>> /home/ec2-user/InfaServiceLog.log
...
then
	createPCServices
     ...
     sh /home/ec2-user/Informatica/9.6.1/isp/bin/infacmd.sh  updategrid -dn $DOMAIN_NAME -un $DOMAIN_USER -pd $domainpass -gn $GRID_NAME -nl  $JOIN_NODE_NAME -ul &>> /home/ec2-user/InfaServiceLog.log

    EXITCODE=$?

     sh /home/ec2-user/Informatica/9.6.1/isp/bin/infacmd.sh  updateServiceProcess -dn $DOMAIN_NAME -un $DOMAIN_USER -pd $domainpass -sn $INTEGRATION_SERVICE_NAME -nn $JOIN_NODE_NAME -po CodePage_Id=106 -ev INFA_CODEPAGENAME=UTF-8 &>> /home/ec2-user/InfaServiceLog.log
